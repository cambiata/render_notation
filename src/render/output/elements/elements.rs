    ...
    for row in matrix.rows.iter() {
        ...
        for (itemidx, (left, right)) in items_iter.tuple_windows().enumerate() {
            if left.is_some() && right.is_some() {
                ...
                let left_coords = (left.coord_x.unwrap(), left.coord_y.unwrap()); //left.coords.expect("RItem coords should always be calculated!");
                let right_coords = (right.coord_x.unwrap(), right.coord_y.unwrap()); //
                let lines_to = &right.lines;
                for (idx, line_to) in lines_to.iter().enumerate() {
                    let rect = NRect::new(
                        left_coords.0,
                        left_coords.1,
                        right_coords.0 - left_coords.0,
