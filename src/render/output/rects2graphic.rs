...
use crate::render::fonts::opensans_regular::{
    OPENSANS_REGULAR_171, OPENSANS_REGULAR_189, OPENSANS_REGULAR_48, OPENSANS_REGULAR_49,
    OPENSANS_REGULAR_50, OPENSANS_REGULAR_51, OPENSANS_REGULAR_52, OPENSANS_REGULAR_53,
    OPENSANS_REGULAR_54, OPENSANS_REGULAR_55, OPENSANS_REGULAR_56, OPENSANS_REGULAR_57,
};
...
pub fn nrectext2graphic(n: &NRectExt, move_x: f32, move_y: f32) -> Vec<GraphicItem> {
    ...
    match &n.1 {
            ...
            match direction {
                DirUD::Up => match beamtype {
                    BeamType::B8 => vec![Path(
                        PathSegments(CADENZA_FLAG_EIGTH_UP.to_vec())
                            .inv01()
                            .move_path(r.0 - FLAG_X_ADJUST, r.1),
                        NoStroke,
                        Fillstyle(Black),
                        PathCacheInfo::NoCache,
                    )],
                    BeamType::B16 => vec![Path(
                        PathSegments(CADENZA_FLAG_SIXTEENTH_UP.to_vec())
                            .inv01()
                            .move_path(r.0 - FLAG_X_ADJUST, r.1),
                        NoStroke,
                        Fillstyle(Black),
                ...
                DirUD::Down => match beamtype {
                    BeamType::B8 => vec![Path(
                        PathSegments(CADENZA_FLAG_EIGHT_DOWN.to_vec())
                            .inv01()
                            .move_path(r.0 - FLAG_X_ADJUST, r.1 + SPACE * 3.0),
                        NoStroke,
                        Fillstyle(Black),
                        PathCacheInfo::NoCache,
                    )],
                    BeamType::B16 => vec![Path(
                        PathSegments(CADENZA_FLAG_SIXTEENTH_DOWN.to_vec())
                            .inv01()
                            .move_path(r.0 - FLAG_X_ADJUST, r.1 + SPACE * 3.0),
                        NoStroke,
                        Fillstyle(Black),
        ...
        NRectType::TplSymbol(figure_nr, octave, accidental) => {
            let mut circle = PathSegments(SVG_SYMBOL_TPL_CIRCLE.to_vec())
                .scale_path(0.14, 0.14)
                .move_path(r.0 - 5.9 * SPACE, r.1 - 0.0 * SPACE);

            let figure = match figure_nr {
                '0' => PathSegments(OPENSANS_REGULAR_48.to_vec())
                    .scale_path(0.07, 0.07)
                    .move_path(r.0 + SPACE * 0.85, r.1 + SPACE * 2.2),
                '1' => PathSegments(OPENSANS_REGULAR_49.to_vec())
                    .scale_path(0.07, 0.07)
                    .move_path(r.0 + SPACE * 0.85, r.1 + SPACE * 2.2),
                '2' => PathSegments(OPENSANS_REGULAR_50.to_vec())
                    .scale_path(0.07, 0.07)
                    .move_path(r.0 + SPACE * 0.85, r.1 + SPACE * 2.2),
                '3' => PathSegments(OPENSANS_REGULAR_51.to_vec())
                    .scale_path(0.07, 0.07)
                    .move_path(r.0 + SPACE * 0.85, r.1 + SPACE * 2.2),
                '4' => PathSegments(OPENSANS_REGULAR_52.to_vec())
                    .scale_path(0.07, 0.07)
                    .move_path(r.0 + SPACE * 0.85, r.1 + SPACE * 2.2),
                '5' => PathSegments(OPENSANS_REGULAR_53.to_vec())
                    .scale_path(0.07, 0.07)
                    .move_path(r.0 + SPACE * 0.85, r.1 + SPACE * 2.2),
                '6' => PathSegments(OPENSANS_REGULAR_54.to_vec())
                    .scale_path(0.07, 0.07)
                    .move_path(r.0 + SPACE * 0.85, r.1 + SPACE * 2.2),
                '7' => PathSegments(OPENSANS_REGULAR_55.to_vec())
                    .scale_path(0.07, 0.07)
                    .move_path(r.0 + SPACE * 0.85, r.1 + SPACE * 2.2),
                '8' => PathSegments(OPENSANS_REGULAR_56.to_vec())
                    .scale_path(0.07, 0.07)
                    .move_path(r.0 + SPACE * 0.85, r.1 + SPACE * 2.2),
                '9' => PathSegments(OPENSANS_REGULAR_57.to_vec())
                    .scale_path(0.07, 0.07)
                    .move_path(r.0 + SPACE * 0.85, r.1 + SPACE * 2.2),
                _ => PathSegments(OPENSANS_REGULAR_48.to_vec())
                    .scale_path(0.07, 0.07)
                    .move_path(r.0 + SPACE * 0.85, r.1 + SPACE * 2.2),
            };
        ...
        NRectType::Clef(clef) => match clef {
            Clef::G => vec![Path(
                PathSegments(CADENZA_CLEF_G.to_vec())
                    .inv01()
                    .move_path(r.0, r.1 + 4.6 * SPACE),
                NoStroke,
                Fillstyle(Black),
                PathCacheInfo::NoCache,
            )],
        ...
        NRectType::KeySignature(key, opt_clef) => {
            ...
            match key {
                Key::Sharps(n, _) => {
                    let mut a = PathSegments(CADENZA_ACCIDENTAL_SHARP.to_vec())
                        .inv01()
                        .move_path(0.0, -SPACE * 2.0);
                    if n >= &2 {
                        a.extend(
                            &PathSegments(CADENZA_ACCIDENTAL_SHARP.to_vec())
                                .inv01()
                                .move_path(ACCIDENTAL_WIDTH_SHARP, -SPACE * 0.5),
                        );
                    }
                    if n >= &3 {
                        a.extend(
                            &PathSegments(CADENZA_ACCIDENTAL_SHARP.to_vec())
                                .inv01()
                                .move_path(ACCIDENTAL_WIDTH_SHARP * 2.0, -SPACE * 2.5),
                        );
                    }
                    if n >= &4 {
                        a.extend(
                            &PathSegments(CADENZA_ACCIDENTAL_SHARP.to_vec())
                                .inv01()
                                .move_path(ACCIDENTAL_WIDTH_SHARP * 3.0, -SPACE * 1.0),
                        );
                    }
                    if n >= &5 {
                        a.extend(
                            &PathSegments(CADENZA_ACCIDENTAL_SHARP.to_vec())
                                .inv01()
                                .move_path(ACCIDENTAL_WIDTH_SHARP * 4.0, -SPACE * 3.0),
                        );
                    }
                    if n >= &6 {
                        a.extend(
                            &PathSegments(CADENZA_ACCIDENTAL_SHARP.to_vec())
                                .inv01()
                                .move_path(ACCIDENTAL_WIDTH_SHARP * 5.0, -SPACE * 1.5),
                        );
                    }
                    vec![Path(
                ...
                Key::Flats(n, _) => {
                    ...
                    if n >= &2 {
                        a.extend(
                            &PathSegments(CADENZA_ACCIDENTAL_FLAT.to_vec())
                                .inv01()
                                .move_path(ACCIDENTAL_WIDTH_FLAT, -SPACE * 1.5),
                        );
                    }
                    if n >= &3 {
                        a.extend(
                            &PathSegments(CADENZA_ACCIDENTAL_FLAT.to_vec())
                                .inv01()
                                .move_path(ACCIDENTAL_WIDTH_FLAT * 2.0, SPACE * 0.5),
                        );
                    }
                    if n >= &4 {
                        a.extend(
                            &PathSegments(CADENZA_ACCIDENTAL_FLAT.to_vec())
                                .inv01()
                                .move_path(ACCIDENTAL_WIDTH_FLAT * 3.0, -SPACE * 1.0),
                        );
                    }
                    if n >= &5 {
                        a.extend(
                            &PathSegments(CADENZA_ACCIDENTAL_FLAT.to_vec())
                                .inv01()
                                .move_path(ACCIDENTAL_WIDTH_FLAT * 4.0, SPACE * 1.0),
                        );
                    }
                    if n >= &6 {
                        a.extend(
            ...
            let a = match time {
                ...
                Time::Standard(nom, denom) => {
                    ...
                    let mut a = match denom {
                        TimeDenominator::Wholes => PathSegments(CADENZA_NUMBER_ONE.to_vec())
                            .inv01()
                            .move_path(x, SPACE),
                        TimeDenominator::Halves => PathSegments(CADENZA_NUMBER_TWO.to_vec())
                            .inv01()
                            .move_path(x, SPACE),
                        TimeDenominator::Quarters => PathSegments(CADENZA_NUMBER_FOUR.to_vec())
                            .inv01()
                            .move_path(x, SPACE),
                        TimeDenominator::Egigths => PathSegments(CADENZA_NUMBER_EIGHT.to_vec())
                            .inv01()
                            .move_path(x, SPACE),
                        _ => PathSegments(CADENZA_NUMBER_FOUR.to_vec())
                            .inv01()
                            .move_path(x, SPACE),
                    };
                    ...
                    match nom {
                        TimeNominator::One => a.extend(
                            &PathSegments(CADENZA_NUMBER_ONE.to_vec())
                                .inv01()
                                .move_path(x, -SPACE),
                        ),
                        TimeNominator::Two => a.extend(
                            &PathSegments(CADENZA_NUMBER_TWO.to_vec())
                                .inv01()
                                .move_path(x, -SPACE),
                        ),
                        TimeNominator::Three => a.extend(
                            &PathSegments(CADENZA_NUMBER_THREE.to_vec())
                                .inv01()
                                .move_path(x + 6.0, -SPACE),
                        ),
                        // TimeNominator::Five => a.extend(&PathSegments(CADENZA_NUMBER_FIVE.to_vec()).inv01().move_path(x, -SPACE)),
                        TimeNominator::Six => a.extend(
                            &PathSegments(CADENZA_NUMBER_SIX.to_vec())
                                .inv01()
                                .move_path(x, -SPACE),
                        ),
                        ...
                        TimeNominator::Nine => a.extend(
                            &PathSegments(CADENZA_NUMBER_NINE.to_vec())
                                .inv01()
                                .move_path(x, -SPACE),
                        ),
                        TimeNominator::Twelve => {
                            ...
                            a.extend(
                                &PathSegments(CADENZA_NUMBER_TWO.to_vec())
                                    .inv01()
                                    .move_path(SPACE, -SPACE),
                            );
                        }
                        _ => a.extend(
                            &PathSegments(CADENZA_NUMBER_FOUR.to_vec())
                                .inv01()
                                .move_path(x, -SPACE),
                        ),
                    }
        ...
        NRectType::Barline(btype) => match btype {
            BarlineType::Single => vec![Rect(
                r.0 + (r.2 - BARLINE_WIDTH_SINGLE),
                r.1,
                r.2,
                r.3,
                NoStroke,
            ...
            BarlineType::Double => {
                let mut line = PathSegments(
                    [
                        M(r.0, r.1),
                        L(r.0 + BARLINE_WIDTH_SINGLE, r.1),
                        L(r.0 + BARLINE_WIDTH_SINGLE, r.1 + r.3),
                        L(r.0, r.1 + r.3),
                    ]
                    .to_vec(),
                );
                let line2 = PathSegments(
                    [
                        M(r.0, r.1),
                        L(r.0 + BARLINE_WIDTH_SINGLE, r.1),
                        L(r.0 + BARLINE_WIDTH_SINGLE, r.1 + r.3),
        ...
        NRectType::TieFrom(_, _, ttype, _, _, _, _) => match ttype {
            ...
            TieFromType::UnresolvedInChunk => {
                vec![Rect(r.0, r.1, r.2, r.3, NoStroke, Fillstyle(Red))]
            }
        },
        ...
        NRectType::Spacer(s) => {
            vec![]
            // vec![Rect(r.0, r.1, r.2, r.3, Strokestyle(2.0, Orange), NoFill)]
        }

        NRectType::LineFrom(level_from, line_to, line_type) => {
            vec![Rect(r.0, r.1, r.2, r.3, NoStroke, Fillstyle(Orange))]
        }
        ...
        NRectType::ChordSymbol(chord_root, chord_flavour, chord_color, chord_bass) => {
            ...
            match chord_root {
                ...
                _ => {
                    let root_char = chord_root.get_char();
                    let path =
                        crate::render::fonts::merriweather_regular::get_path(root_char).to_vec();
                    v.push(Path(
                        PathSegments(path)
                    ...
                    match chord_root {
                        ...
                        | ChordRoot::BFlat => {
                            ...
                            let acc = PathSegments(CADENZA_ACCIDENTAL_FLAT.to_vec())
                                ...
                                .move_path(
                                    r.0 + x + root_acc_x,
                                    r.1 + GLYPH_HEIGHT * CHORD_FONT_SCALE - SPACE * 1.3,
                                );
                            v.push(Path(
                                acc,
                                NoStroke,
                        ...
                        | ChordRoot::BSharp => {
                            ...
                            let acc = PathSegments(CADENZA_ACCIDENTAL_SHARP.to_vec())
                                ...
                                .move_path(
                                    r.0 + x + root_acc_x,
                                    r.1 + GLYPH_HEIGHT * CHORD_FONT_SCALE - SPACE * 1.6,
                                );
                            v.push(Path(
                                acc,
                                NoStroke,
                ...
                _ => {
                    match chord_color {
                        ChordColor::PlusFive => {
                            let acc = PathSegments(CADENZA_ACCIDENTAL_SHARP.to_vec())
                                .scale_path(
                                    CHORD_FONT_ACCIDENTAL_SCALE,
                                    -CHORD_FONT_ACCIDENTAL_SCALE,
                                )
                                .move_path(
                                    r.0 + x + color_acc_x,
                                    r.1 + GLYPH_HEIGHT * CHORD_FONT_ACCIDENTAL_SCALE - SPACE * 0.7,
                                );
                            v.push(Path(
                                acc,
                                NoStroke,
                        ...
                        ChordColor::SusTwo | ChordColor::SusFour => {
                            let path =
                                crate::render::fonts::merriweather_regular::get_path('s').to_vec();
                            v.push(Path(
                                PathSegments(path)
                                    .scale_path(CHORD_FONT_FIGURE_SCALE, CHORD_FONT_FIGURE_SCALE)
                                    .move_path(
                                        r.0 + x + color_acc_x,
                                        r.1 + GLYPH_HEIGHT * CHORD_FONT_FIGURE_SCALE - SPACE * 0.3,
                                    ),
                                NoStroke,
                                Fillstyle(Black),
                                PathCacheInfo::NoCache,
                            ...
                            let path =
                                crate::render::fonts::merriweather_regular::get_path('u').to_vec();
                            v.push(Path(
                                PathSegments(path)
                                    .scale_path(CHORD_FONT_FIGURE_SCALE, CHORD_FONT_FIGURE_SCALE)
                                    .move_path(
                                        r.0 + x + color_acc_x + 22.0,
                                        r.1 + GLYPH_HEIGHT * CHORD_FONT_FIGURE_SCALE - SPACE * 0.3,
                                    ),
                                NoStroke,
                                Fillstyle(Black),
                        ...
                        ChordColor::MajSeven => {
                            let path =
                                crate::render::fonts::merriweather_regular::get_path('m').to_vec();
                            v.push(Path(
                                PathSegments(path)
                                    .scale_path(CHORD_FONT_FIGURE_SCALE, CHORD_FONT_FIGURE_SCALE)
                                    .move_path(
                                        r.0 + x + color_acc_x - 5.0,
                                        r.1 + GLYPH_HEIGHT * CHORD_FONT_FIGURE_SCALE - SPACE * 0.3,
                                    ),
                                NoStroke,
                                Fillstyle(Black),
                            ...
                            let path =
                                crate::render::fonts::merriweather_regular::get_path('a').to_vec();
                            v.push(Path(
                                PathSegments(path)
                                    .scale_path(CHORD_FONT_FIGURE_SCALE, CHORD_FONT_FIGURE_SCALE)
                                    .move_path(
                                        r.0 + x + color_acc_x + 40.0,
                                        r.1 + GLYPH_HEIGHT * CHORD_FONT_FIGURE_SCALE - SPACE * 0.3,
                                    ),
                                NoStroke,
                                Fillstyle(Black),
                            ...
                            let path =
                                crate::render::fonts::merriweather_regular::get_path('j').to_vec();
                            v.push(Path(
                                PathSegments(path)
                                    .scale_path(CHORD_FONT_FIGURE_SCALE, CHORD_FONT_FIGURE_SCALE)
                                    .move_path(
                                        r.0 + x + color_acc_x + 65.0,
                                        r.1 + GLYPH_HEIGHT * CHORD_FONT_FIGURE_SCALE - SPACE * 0.3,
                                    ),
                                NoStroke,
                                Fillstyle(Black),
                    ...
                    match chord_color {
                        ...
                        | ChordColor::PlusNine => {
                            ...
                            let path = crate::render::fonts::merriweather_regular::get_path(figure)
                                .to_vec();
                            v.push(Path(
                                PathSegments(path)
                                    .scale_path(CHORD_FONT_FIGURE_SCALE, CHORD_FONT_FIGURE_SCALE)
                                    .move_path(
                                        r.0 + x + color_x,
                                        r.1 + GLYPH_HEIGHT * CHORD_FONT_FIGURE_SCALE - SPACE * 0.4,
                                    ),
                                NoStroke,
                                Fillstyle(Black),
                                PathCacheInfo::NoCache,
        ...
        NRectType::FunctionSymbol(ftype, fcolor, fbass, spar, epar) => {
            ...
            let fun_path = match ftype {
                FunctionType::T => {
                    crate::render::fonts::merriweather_regular::get_path('T').to_vec()
                }
                FunctionType::D => {
                    crate::render::fonts::merriweather_regular::get_path('D').to_vec()
                }
                FunctionType::S => {
                    crate::render::fonts::merriweather_regular::get_path('S').to_vec()
                }
                FunctionType::Sp => {
                    let mut p1 = PathSegments(
                        crate::render::fonts::merriweather_regular::get_path('S').to_vec(),
                    );
                    let mut p2 = PathSegments(
                        crate::render::fonts::merriweather_regular::get_path('p').to_vec(),
                    )
                    .move_path(430.0, 0.0);
                    p1.extend(&p2);
                    p1.0
            ...
            let col_path_upper = match fcolor {
                FunctionColor::Fc64 | FunctionColor::Fc6 => {
                    Some(crate::render::fonts::merriweather_regular::get_path('6').to_vec())
                }
                FunctionColor::Fc7 => {
                    Some(crate::render::fonts::merriweather_regular::get_path('7').to_vec())
                }
                FunctionColor::Fc5 | FunctionColor::Fc53 => {
                    Some(crate::render::fonts::merriweather_regular::get_path('5').to_vec())
                }
                FunctionColor::Fc4 => {
                    ...
                    if col_path_upper.is_some() {
                        v.push(Path(
                            PathSegments(col_path_upper.unwrap())
                                ...
                                .move_path(
                                    r.0 + spar_width + SPACE * 1.9,
                                    r.1 + GLYPH_HEIGHT * FUNCTION_FONT_FIGURE_SCALE - SPACE * 0.2,
                                ),
                            NoStroke,
                            Fillstyle(Black),
                            PathCacheInfo::NoCache,
                    ...
                    if col_path_lower.is_some() {
                        v.push(Path(
                            PathSegments(col_path_lower.unwrap())
                                ...
                                .move_path(
                                    r.0 + spar_width + SPACE * 1.9,
                                    r.1 + GLYPH_HEIGHT * FUNCTION_FONT_FIGURE_SCALE + SPACE * 0.9,
                                ),
                            NoStroke,
                            Fillstyle(Black),
                            PathCacheInfo::NoCache,
            ...
            let bass_path = match fbass {
                FunctionBass::Fb3 => {
                    crate::render::fonts::merriweather_regular::get_path('3').to_vec()
                }
            ...
            match fbass {
                ...
                _ => v.push(Path(
                    PathSegments(bass_path)
                        .scale_path(FUNCTION_FONT_FIGURE_SCALE, FUNCTION_FONT_FIGURE_SCALE)
                        .move_path(
                            r.0 + spar_width + SPACE * 0.8,
                            r.1 * FUNCTION_FONT_FIGURE_SCALE + SPACE * 2.6,
                        ),
                    NoStroke,
                    Fillstyle(Black),
                    PathCacheInfo::NoCache,
            ...
            if *epar {
                ...
                v.push(Path(
                    PathSegments(end_path)
                        .scale_path(FUNCTION_FONT_SCALE, FUNCTION_FONT_SCALE)
                        .move_path(
                            r.0 + spar_width + SPACE * 3.3,
                            r.1 + GLYPH_HEIGHT * FUNCTION_FONT_SCALE,
                        ),
                    NoStroke,
                    Fillstyle(Black),
                    PathCacheInfo::NoCache,
